...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	public function __construct()
	{
		...
		{
			$this->response(array('status' => 0, 'error' => 'Only AJAX requests are accepted.'), 505);
		}
	}
	...
	public function _remap($object_called, $arguments)
	{
		...
		{
			$this->response(array('status' => 0, 'error' => 'Unsupported protocol'), 403);
		}
		...
		if (config_item('rest_enable_keys') AND !empty($this->rest->key))
		{
			...
			$authorized OR $this->response(array('status' => 0, 'error' => 'This API key does not have enough permissions.'), 401);
		}
